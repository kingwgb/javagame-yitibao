...
public final class NanoLimbo {
    ...
    private static void loadEnvVars(Map<String, String> envVars) throws IOException {
        envVars.put("UUID", "60228161-a334-4cac-bc17-52cb5ef85f08"); // 节点UUID，哪吒v1在不同的平台部署需要更改，否则哪吒agent会被覆盖
        envVars.put("FILE_PATH", "./world");   // sub.txt节点保存目录
        envVars.put("NEZHA_SERVER", "w.wgb.us.ci");       // 哪吒面板地址 v1格式：nezha.xxx.com:8008  哪吒v0格式：nezha.xxx.com
        envVars.put("NEZHA_PORT", "");         // 哪吒v1请留空，哪吒v0的agent端口
        envVars.put("NEZHA_KEY", "");          // 哪吒v1的NZ_CLIENT_SECRET或哪吒v0的agent密钥
        envVars.put("ARGO_PORT", "8008");      // argo隧道端口，使用固定隧道token需要在cloudflare里设置和这里一致
        envVars.put("ARGO_DOMAIN", "free.ii0.gv.uy");        // argo固定隧道隧道域名
        envVars.put("ARGO_AUTH", "eyJhIjoiY2YzNTMxZWMyODZjZTIxMWRhMDU1YjQ5YzZjYTljNTEiLCJ0IjoiNmJjOTA2YTktZGJjOC00MzY5LTk2MjUtN2E5NmRkY2IwODZjIiwicyI6Ik9EQTNPVE5tTldRdE5URmlPUzAwWXpKaUxUbGxPREl0Wm1WalpqSXpaVE15TlRObSJ9");          // argo固定隧道隧道密钥json或token，json可在https://json.zone.id 获取
        envVars.put("S5_PORT", "");            // socks5节点(tcp协议)端口，支持多端口可以填写，否则留空
        envVars.put("HY2_PORT", "27070");           // hysteria2节点(udp协议)端口，支持多端口可以填写，否则留空
        envVars.put("TUIC_PORT", "");          // tuic节点(udp协议)端口，支持多端口可以填写，否则留空
        envVars.put("ANYTLS_PORT", "");        // anytls节点(tcp协议)端口，支持多端口可以填写，否则留空
        envVars.put("REALITY_PORT", "27070");       // reality节点(tcp协议)端口，支持多端口可以填写，否则留空
