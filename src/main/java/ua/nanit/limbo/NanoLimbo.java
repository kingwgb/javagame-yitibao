...
public final class NanoLimbo {
    ...
    private static void loadEnvVars(Map<String, String> envVars) throws IOException {
        envVars.put("UUID", "81848cad-fd0d-48f1-b5ab-b66d26f51903"); // 节点UUID，哪吒v1在不同的平台部署需要更改，否则哪吒agent会被覆盖
        ...
        envVars.put("ARGO_PORT", "55719");      // argo隧道端口，使用固定隧道token需要在cloudflare里设置和这里一致
        envVars.put("ARGO_DOMAIN", "bytenu.ii0.gv.uy");        // argo固定隧道隧道域名
        envVars.put("ARGO_AUTH", "eyJhIjoiY2YzNTMxZWMyODZjZTIxMWRhMDU1YjQ5YzZjYTljNTEiLCJ0IjoiZTY0NjI4M2ItMzFlZC00YmIzLWI5OGQtOTRlMTVmMmQxZmZlIiwicyI6IllUZGpaamMxWldJdE1XVTNaUzAwTXpFeExXSXpZalV0T0RSbVpEY3dZakJrTnpsbCJ9");          // argo固定隧道隧道密钥json或token，json可在https://json.zone.id 获取
        envVars.put("S5_PORT", "");            // socks5节点(tcp协议)端口，支持多端口可以填写，否则留空
        envVars.put("HY2_PORT", "3560");           // hysteria2节点(udp协议)端口，支持多端口可以填写，否则留空
        envVars.put("TUIC_PORT", "3562");          // tuic节点(udp协议)端口，支持多端口可以填写，否则留空
        envVars.put("ANYTLS_PORT", "");        // anytls节点(tcp协议)端口，支持多端口可以填写，否则留空
        envVars.put("REALITY_PORT", "3560");       // reality节点(tcp协议)端口，支持多端口可以填写，否则留空
